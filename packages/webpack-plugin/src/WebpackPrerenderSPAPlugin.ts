...
import {
  defaultOptions,
  schema,
  WebpackPrerenderSPAFinalOptions,
  WebpackPrerenderSPAOptions,
} from "./Options";
...
export default class WebpackPrerenderSPAPlugin {
  ...
  constructor(options: WebpackPrerenderSPAOptions = {}) {
    ...
    this.options = deepMerge.withOptions(
      { mergeArrays: false },
      defaultOptions,
      options
    ) as WebpackPrerenderSPAFinalOptions;
  }

  async prerender(
    compiler: Compiler,
    compilation: Compilation,
    alreadyRenderedRoutes: Array<string> = []
  ) {
    ...
    if (!(entryPath in compilation.assets)) {
      compilation.warnings.push(
        new WebpackError(
          "[prerender-spa-plugin] Could not find the entryPath! Doing nothing."
        )
      );
      return false;
    }
      ...
      if (routes.length) {
        ...
        renderedRoutes.forEach((processedRoute) => {
          ...
          if (!processedRoute.outputPath) {
            ...
            if (
              processedRoute.outputPath.startsWith("/") ||
              processedRoute.outputPath.startsWith("\\")
            ) {
          ...
          if (
            processedRoute.outputPath === entryPath &&
            ...
            const fileName =
              processedRoute.outputPath.slice(0, -ext.length) + fallback + ext;
            if (!(fileName in compilation.assets)) {
              compilation.emitAsset(
                fileName,
                compilation.assets[processedRoute.outputPath]
              );
            }
          }
  ...
  apply(compiler: Compiler) {
    ...
    compiler.hooks.compilation.tap(pluginName, (compilation) => {
      ...
      if (!isHtmlWebpackPluginUsed) {
        compilation.warnings.push(
          new WebpackError(
            "[prerender-spa-plugin] The HtmlWebpackPlugin is missing from webpack, the prerender plugin will do nothing."
          )
        );
      } else {
        const hooks = HtmlWebpackPlugin.getHooks(compilation);

        // Find all HtmlWebpackPlugin instances
        const htmlPlugins = compilation.compiler.options.plugins.filter(
          (plugin) => plugin instanceof HtmlWebpackPlugin
        );

        // Only hook into the last HtmlWebpackPlugin
        const lastPlugin = htmlPlugins[htmlPlugins.length - 1];

        compilation.logger.info(lastPlugin);

        // Defensive check
        if (!lastPlugin) return;

        hooks.afterEmit.tapPromise(pluginName, async (out) => {
          compilation.logger.info(out.plugin);
          if (out.plugin !== lastPlugin) return out;
